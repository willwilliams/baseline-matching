...
function train()
   ...
   for i=1,opt.epochSize*optimator:getNumMachines() do
      ...
      local childProcess = optimator:getChildProcessByIndex(optimator:getNumMachines - (-i%optimator:getNumMachines))
      local model = optimator:extractModel()
      donkeys:addjob(
         -- the job callback (runs in data-worker thread)
         function()
            workerModel = model
            workerChild = childProcess
            local inputs, labels = trainLoader:sample(opt.batchSize)
            local computeObject = {model = self.model, inputs = inputCPU, labels = labelsCPU}
            workerChild:send(computeObject)
         end,
